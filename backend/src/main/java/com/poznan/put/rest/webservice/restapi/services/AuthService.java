...
public class AuthService {
    ...
    public AuthService(StudentService studentService, TutorService tutorService) {
    ...
    public Student registerStudent(RegisterRequest registerRequest) {
        ...
        if (tutorService.findByEmail(registerRequest.email()).isPresent()) {
        ...
        if (tutorService.findByPhoneNumber(registerRequest.phoneNumber()).isPresent()) {
    ...
    public Student studentLogin(LoginRequest loginRequest) {
        return studentService.findByEmailAndPassword(loginRequest.email(), loginRequest.password())
                .orElseThrow(() -> new BadCredentialsException("Invalid student credentials"));
    }

    public Tutor tutorLogin(LoginRequest loginRequest) {
        return tutorService.findByEmailAndPassword(loginRequest.email(), loginRequest.password())
